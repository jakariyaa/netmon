...
bool writeFile(const char *outputFile, const std::ostringstream &data)
{
    ...
    if (fd >= 0)
    {
        ...
        if (write(fd, buffer.c_str(), buffer.length()) == (ssize_t)buffer.length())
            success = true;
        close(fd);
    }
    return success;
...
std::ostream &operator<<(std::ostream &out, const std::map<K, V> &container)
{
    for (const auto &item : container)
        out << item.first << '|' << item.second << '\t'; // write into storage
    return out << '\n';
}

template <typename T>
bool fromString(const std::string &from, T &to) { return bool(std::istringstream(from) >> to); }
    ...
    void readProc()
    {
        ...
        while (cpustat >> name)
        {
            if (name.find("cpu") == 0)
            {
                ...
                if (name.empty())
                    number = -1;
                else if (!fromString(name, number))
                    continue;
                Core &core = cores[number];
                cpustat >> core.user >> core.nice >> core.system >> core.idle >> core.iowait >> core.irq >> core.softirq;
                if (cpustat.peek() != '\n')
                    cpustat >> core.steal;
                else
                    core.steal = 0; // caution with old kernels
                if (cpustat.peek() != '\n')
                    cpustat >> core.guest;
                else
                    core.guest = 0;
        ...
        if (allcpu != cores.end())
            allcpu->second.freq_hz = sum_freq;
    }
};
...
{
    return in >> core.user >> core.nice >> core.system >> core.idle >> core.iowait >> core.irq >> core.softirq >> core.steal >> core.guest >> core.guestnice >> core.freq_hz;
}
...
struct Memory
{
    ...
    void readProc()
    {
        ...
        for (int count = 0; meminfo >> key;)
        {
            ...
            if (count == 7 + (hasAvailable ? 1 : 0))
                break;
            meminfo.ignore(buffer.size(), '\n');
        }
        if (!hasAvailable)
            ram.available = ram.free + ram.buffers + ram.cached; // pre-2014 kernels
    }
};
...
{
    return in >> ram.total >> ram.available >> ram.free >> ram.shared >> ram.buffers >> ram.cached >> ram.swapTotal >> ram.swapFree;
}
...
struct IO
{
    ...
    void readProc()
    {
        ...
        {
            uint64_t skip;
            if (!(diskinfo >> skip >> skip >> name))
                break;
            if (!name.empty() && (prev.empty() || (name.find(prev) != 0)) // skip partitions
                && (name.find("dm") != 0))                                // skip device mapper
            {
                prev = name;
                Device &device = devices[name];
                uint64_t sectorsRd, sectorsWr;
                diskinfo >> skip >> skip >> sectorsRd >> skip >> skip >> skip >> sectorsWr >> skip >> skip >> device.ioMsecs;
        ...
        std::string skip;
        bool hasData = false;
        while (!hasData && std::getline(partinfo, skip))
            if (skip.empty())
                hasData = true;
        if (hasData)
            while (partinfo >> skip >> skip >> blocks >> name)
            {
                auto pdev = devices.find(name);
                if (pdev != devices.end())
                    pdev->second.bytesSize = blocks * 1024;
...
struct Network
{
    ...
    void readProc()
    {
        ...
        for (;;)
        {
            netinfo.ignore(buffer.size(), '\n');
            std::string name;
            if (!std::getline(netinfo, name, ':'))
                break; // also handles kernels not having a space after ':'
            while (!name.empty() && (name.front() == ' '))
                name.erase(name.begin()); // ltrim
            Interface &interface = interfaces[name];
            uint64_t skip;
            netinfo >> interface.bytesRecv >> skip >> skip >> skip >> skip >> skip >> skip >> skip >> interface.bytesSent;
        }
    }
};
...
std::ostream &operator<<(std::ostream &out, const Network::Bandwidth &speed)
{
    ...
    if (speed.perSecond < MB_i)
        return out << speed.perSecond / 1000 << " K" << unit << "ps";
...
struct Health
{
    ...
    void readProc()
    {
        ...
        for (int hwmon = 0; hwmon < 256; hwmon++)
        {
            ...
            {
                base.append("/device");
                if (!readFile(VA_STR(base << "/name").c_str(), buffer, false))
                    break;
            }
            ...
            for (int ic = 1; ic < 64; ic++)
            {
                ...
                if (!readFile(VA_STR(coretemp << "/temp" << ic << "_label").c_str(), buffer, false))
                {
                    if (thermometers.empty())
                        ...
                        break; // Atom CPU may start at 2 (!?)
                }
                std::istringstream sname(&buffer[0]);
                std::string name;
                if (!std::getline(sname, name) || name.empty())
                    break;
                if (!readFile(VA_STR(coretemp << "/temp" << ic << "_input").c_str(), buffer, false))
                    break;
                std::istringstream temp(&buffer[0]);
                int32_t tempMilliCelsius;
                if (!(temp >> tempMilliCelsius))
                    break;
...
std::ostream &operator<<(std::ostream &out, const DataSize &data)
{
    if (data.bytes < 5000)
        return out << "0 MB";
    if (data.bytes < 10 * MB_i)
        return out << std::fixed << std::setprecision(2) << data.bytes / MB_f << " MB";
    if (data.bytes < 100 * MB_i)
        return out << std::fixed << std::setprecision(1) << data.bytes / MB_f << " MB";
    if (data.bytes < GB_i)
        return out << data.bytes / MB_i << " MB";
    if (data.bytes < 10 * GB_i)
        return out << std::fixed << std::setprecision(2) << data.bytes / GB_f << " GB";
    if (data.bytes < 100 * GB_i)
        return out << std::fixed << std::setprecision(1) << data.bytes / GB_f << " GB";
    if (data.bytes < TB_i)
        return out << data.bytes / GB_i << " GB";
    uint64_t tbx100 = data.bytes / (TB_i / 100);
    auto decimals = (tbx100 > 9999) || (tbx100 % 100 == 0) ? 0 : (tbx100 % 10 == 0) ? 1
                                                                                    : 2; // pretty disk sizes
...
template <typename T>
std::ostream &operator<<(std::ostream &out, const Padded<T> &data)
{
    if (!std::isnan(data.value))
        for (T div = data.max;; div /= 10) // avoid infinite loop with NaN numbers
        {
            if ((data.value >= div) && (data.value >= 1))
                break;
            if ((data.value < 1) && (div <= 1))
                break;
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if ((arg == "LINE"))
            ...
            posRam = i, new_Memory.reset(new Memory());
        else if ((arg == "IO"))
            new_IO.reset(new IO());
        else if ((arg == "NET"))
            new_Network.reset(new Network());
        else if ((arg == "NET8"))
            new_Network.reset(new Network()), netSpeedUnit = Network::Bandwidth::Unit::byte;
        else if ((arg == "TEMP"))
            posTemp = i, new_Health.reset(new Health());
        else
        {
            new_Network.reset(new Network());
    ...
    if (new_Network && old_Network && nsecsElapsed) // NET report
    {
        if (selectedNetworkInterface.empty())
        {
            ...
            for (auto itn = new_Network->interfaces.cbegin(); itn != new_Network->interfaces.cend(); ++itn)
            {
                ...
                if (ito == old_Network->interfaces.end())
                    continue;
        ...
        for (auto itn = new_Network->interfaces.cbegin(); itn != new_Network->interfaces.cend(); ++itn)
        {
            ...
            if (!nif.traffic() && !isSelectedInterface)
                continue;

            auto dumpNet = [&](uint64_t newBytes, uint64_t oldBytes)
            {
                int64_t delta = newBytes - oldBytes;
                int64_t speed = (netSpeedUnit == Network::Bandwidth::Unit::byte ? 1 : 8) * delta / secsElapsed;
                reportDetail << "    " << DataSize{newBytes};
                if (speed > 0)
                    reportDetail << " - " << Network::Bandwidth{netSpeedUnit, speed};
                reportDetail << " \n";
                if (isSelectedInterface)
                    reportStd << std::setw(6) << Network::Bandwidth{netSpeedUnit, speed}
                              << (singleLine ? " " : " \n");
            };

            reportDetail << " " << itn->first << ": ";
            if (isSelectedInterface)
                reportDetail << "\u2713"; // "check mark" character
            reportDetail << "\n";
    ...
    if (new_CPU && old_CPU) // CPU report
    {
        ...
        if ((allnew != new_CPU->cores.end()) && (allold != old_CPU->cores.end()))
        {
            ...
            if (cpuTotal > 0)
            {
                ...
                if (ncpu.irq)
                    dumpPercent("irq", diff.irq, ncpu.irq);
                if (ncpu.softirq)
                    dumpPercent("softirq", diff.softirq, ncpu.softirq);
                if (ncpu.steal)
                    dumpPercent("steal", diff.steal, ncpu.steal);
                if (ncpu.guest)
                    dumpPercent("guest", diff.guest, ncpu.guest);
                if (ncpu.guestnice)
                    dumpPercent("guest nice", diff.guestnice, ncpu.guestnice);
        ...
        for (auto nitd = new_IO->devices.cbegin(); nitd != new_IO->devices.cend(); ++nitd)
        {
            ...
            if ((device.bytesRead || device.bytesWritten) && (prevdev != old_IO->devices.end()))
            {
                ...
                auto dumpIO = [&](const char *iconIdle, const char *iconBusy, uint64_t newBytes, uint64_t oldBytes)
                {
                    ...
                    if (transferred)
                        reportDetail << " - " << IO::Bandwidth{transferred / secsElapsed};
                    reportDetail << " \n";
                };
    ...
    if (new_Health) // TEMP report
    {
        ...
        for (const auto &itt : new_Health->thermometers)
        {
            ...
            if (itt.second.tempMilliCelsius < its->second.min)
                its->second.min = itt.second.tempMilliCelsius;
            if (itt.second.tempMilliCelsius > its->second.max)
                its->second.max = itt.second.tempMilliCelsius;
            if (itt.second.tempMilliCelsius > maxAbsTemp)
                maxAbsTemp = itt.second.tempMilliCelsius;
        ...
        if (!statByCategory.empty())
            reportDetail << " Temperature: \n";
